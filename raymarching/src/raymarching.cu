...
// mask_out: [N, K]
template <typename scalar_t>
__global__ void kernel_composite_rays_with_masks_train_forward(
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs,  
    const scalar_t * __restrict__ masks,
    const scalar_t * __restrict__ deltas,
    const int * __restrict__ rays,
    const uint32_t M, const uint32_t N, const uint32_t K, const float T_thresh, 
    scalar_t * weights_sum,
    scalar_t * depth,
    scalar_t * image,
    scalar_t * mask_out
...
void composite_rays_with_masks_train_forward(const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor masks, const at::Tensor deltas, const at::Tensor rays, const uint32_t M, const uint32_t N, const uint32_t K, const float T_thresh, at::Tensor weights_sum, at::Tensor depth, at::Tensor image, at::Tensor mask_out) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    sigmas.scalar_type(), "composite_rays_with_masks_train_forward", ([&] {
        kernel_composite_rays_with_masks_train_forward<<<div_round_up(N, N_THREAD), N_THREAD>>>(sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), masks.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), M, N, K, T_thresh, weights_sum.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), mask_out.data_ptr<scalar_t>());
    }));
}
...
// grad_masks: [M, K]
template <typename scalar_t>
__global__ void kernel_composite_rays_with_masks_train_backward(
    const scalar_t * __restrict__ grad_weights_sum,
    const scalar_t * __restrict__ grad_image,
    const scalar_t * __restrict__ grad_mask_out,
    const scalar_t * __restrict__ sigmas,
    const scalar_t * __restrict__ rgbs, 
    const scalar_t * __restrict__ masks,
    const scalar_t * __restrict__ deltas,
    ...
    const scalar_t * __restrict__ weights_sum,
    const scalar_t * __restrict__ image,
    const scalar_t * __restrict__ mask_out,
    const uint32_t M, const uint32_t N, const uint32_t K, const float T_thresh,
    scalar_t * grad_sigmas,
    scalar_t * grad_rgbs,
    scalar_t * grad_masks_acc,  // workaround for accumulation
    scalar_t * grad_masks
) {
    // parallel per ray
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    ...
    while (step < num_steps) {
        ...
        for (uint32_t k = 0; k < K; k++) {
            grad_masks_acc[k] += weight * masks[k];
        }
        ...
        // write grad_masks
        for (uint32_t k = 0; k < K; k++) {
            grad_masks[k] = grad_mask_out[k] * weight;
        }
        ...
        for (uint32_t k = 0; k < K; k++) {
            grad_sigmas[0] += deltas[0] * (
                grad_mask_out[k] * (T * masks[k] - (mask_out[k] - grad_masks_acc[k]))
            );
        }
...
void composite_rays_with_masks_train_backward(const at::Tensor grad_weights_sum, const at::Tensor grad_image, const at::Tensor grad_mask_out, const at::Tensor sigmas, const at::Tensor rgbs, const at::Tensor masks, const at::Tensor deltas, const at::Tensor rays, const at::Tensor weights_sum, const at::Tensor image, const at::Tensor mask_out, const uint32_t M, const uint32_t N, const uint32_t K, const float T_thresh, at::Tensor grad_sigmas, at::Tensor grad_rgbs, at::Tensor grad_masks_acc, at::Tensor grad_masks) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    grad_image.scalar_type(), "composite_rays_with_masks_train_backward", ([&] {
        kernel_composite_rays_with_masks_train_backward<<<div_round_up(N, N_THREAD), N_THREAD>>>(grad_weights_sum.data_ptr<scalar_t>(), grad_image.data_ptr<scalar_t>(), grad_mask_out.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), masks.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), rays.data_ptr<int>(), weights_sum.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), mask_out.data_ptr<scalar_t>(), M, N, K, T_thresh, grad_sigmas.data_ptr<scalar_t>(), grad_rgbs.data_ptr<scalar_t>(), grad_masks_acc.data_ptr<scalar_t>(), grad_masks.data_ptr<scalar_t>());
    }));
}
...
template <typename scalar_t>
__global__ void kernel_composite_rays_with_masks(
    const uint32_t n_alive, 
    const uint32_t n_step, 
    const uint32_t K,
    const float T_thresh,
    ...
    const scalar_t* __restrict__ masks,
    const scalar_t* __restrict__ deltas, 
    scalar_t* weights_sum, scalar_t* depth, scalar_t* image, scalar_t* mask_out
) {
    const uint32_t n = threadIdx.x + blockIdx.x * blockDim.x;
    if (n >= n_alive) return;
...
void composite_rays_with_masks(const uint32_t n_alive, const uint32_t n_step, const uint32_t K, const float T_thresh, at::Tensor rays_alive, at::Tensor rays_t, at::Tensor sigmas, at::Tensor rgbs, at::Tensor masks, at::Tensor deltas, at::Tensor weights, at::Tensor depth, at::Tensor image, at::Tensor mask_out) {
    static constexpr uint32_t N_THREAD = 128;
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
    image.scalar_type(), "composite_rays_with_masks", ([&] {
        kernel_composite_rays_with_masks<<<div_round_up(n_alive, N_THREAD), N_THREAD>>>(n_alive, n_step, K, T_thresh, rays_alive.data_ptr<int>(), rays_t.data_ptr<scalar_t>(), sigmas.data_ptr<scalar_t>(), rgbs.data_ptr<scalar_t>(), masks.data_ptr<scalar_t>(), deltas.data_ptr<scalar_t>(), weights.data_ptr<scalar_t>(), depth.data_ptr<scalar_t>(), image.data_ptr<scalar_t>(), mask_out.data_ptr<scalar_t>());
    }));
}
