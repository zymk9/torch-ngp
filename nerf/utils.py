...
def preprocess_feature(feature, H, W):
    f_h, f_w = feature.shape[-2], feature.shape[-1]
    max_size = max(W, H)
...
class MSEMeter:
    ...
    def update(self, preds, truths):
        preds, truths = self.prepare_inputs(preds, truths) # [B, N, C] or [B, H, W, C]
        errors = np.mean(np.square(preds - truths))
        
        self.V += np.mean(errors)
        self.N += 1
