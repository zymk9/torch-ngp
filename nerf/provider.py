...
class NeRFSemanticDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test_per_pose=10, n_test_poses=10):
        ...
        self.num_instances = 41 # nyu40 + 1, this is actually the number of semantics

        # load transform
        _type = self.type if self.type!='test' else 'val'
        with open(os.path.join(self.root_path, _type, f'transforms.json'), 'r') as f:
            transform = json.load(f)
        ...
        # read images, poses, semantics
        frames = transform["frames"]
            
        self.poses = []
        self.images = []
        self.semantics = []
        for f in tqdm.tqdm(frames, desc=f'Loading {_type} data'):
            f_path = os.path.join(self.root_path, 'train', f['file_path'])
            ...
            scene_name = self.root_path.split('/')[-1]
            frame_name = f['file_path'].split('/')[-1][:-4]
            semantic = np.load(os.path.join(self.root_path, '../../pred_semantic', scene_name, frame_name + '.npy')).astype(np.float32)
        ...
        if self.semantics is not None:
            self.semantics = torch.from_numpy(np.stack(self.semantics, axis=0))
        ...
        if self.preload:
            ...
            if self.semantics is not None:
                self.semantics = self.semantics.to(torch.long).to(self.device)
    ...
    def collate(self, index):
        ...
        if self.semantics is not None:
            ...
            if self.training:
                semantics = torch.gather(semantics.view(B, -1), 1, rays['inds']) # [B, N]
            results['masks'] = semantics
    ...
    def dataloader(self):
        ...
        loader = DataLoader(list(range(size)), batch_size=1, collate_fn=self.collate, shuffle=self.training, num_workers=0)
        loader._data = self # an ugly fix... we need to access error_map & poses in trainer.
        loader.has_gt = (self.semantics is not None) and (self.images is not None)
        return loader
